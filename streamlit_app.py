...
if solve_btn or st.session_state.locked:
    ...
    with col_left:
        ...
        # Burchak nuqtalarini topish (Шаг 0)
        corner_points = []
        ...
        if corner_points:
            ...
            fig.add_trace(go.Scatter(x=[center[0]], y=[center[1]], mode='markers', marker=dict(color='blue', size=10), name="Шаг 2: Внутр. точка"))
            if abs(c_main2) > 1e-7:
                y_inner = (inner_z - c_main1 * x_range) / c_main2
                fig.add_trace(go.Scatter(x=x_range, y=y_inner, mode='lines', line=dict(color='blue', dash='dot'), name="Шаг 3: Линия уровня"))
        ...
        fig.update_layout(height=700, xaxis=dict(range=[-10, 10], zeroline=True, zerolinecolor='black'), yaxis=dict(range=[-10, 10], zeroline=True, zerolinecolor='black'), plot_bgcolor='white')
...
for h in st.session_state.history:
    st.info(f"🕒 `{h['time']}` | **Z: {h['z']:.2f}** | X: {h['x']:.2f}, Y: {h['y']:.2f} ({h['type']})")import streamlit as st
import numpy as np
import plotly.graph_objects as go
from scipy.optimize import linprog
from fpdf import FPDF
...
st.header(t_hist)
for h in st.session_state.history:
    st.info(f"🕒 `{h['time']}` | **Z: {h['z']:.2f}** | X: {h['x']:.2f}, Y: {h['y']:.2f} ({h['type']})")v
