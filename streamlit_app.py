...
st.markdown("<h1 style='text-align: center;'>📊 Chiziqli dasturlash — Yechuvchi</h1>", unsafe_allow_html=True)
...
def create_pdf(opt_x, opt_y, opt_val, obj_type):
    ...
    pdf.cell(200, 10, txt="Chiziqli dasturlash masalasi hisoboti", ln=True, align='C')
    pdf.ln(10)
    pdf.set_font("Arial", size=12)
    pdf.cell(200, 10, txt=f"Maqsad funksiyasi turi: {obj_type}", ln=True)
    pdf.cell(200, 10, txt=f"X = {opt_x:.2f}, Y = {opt_y:.2f}", ln=True)
    pdf.cell(200, 10, txt=f"Natija Z = {opt_val:.2f}", ln=True)
...
with st.sidebar:
    ...
    with col_t:
        obj_type = st.selectbox("Turi", ("max", "min"), key="main_type", label_visibility="collapsed")
    ...
    st.markdown("---")
    solve_btn = st.button("🚀 Hisoblash", type="primary", use_container_width=True)
    ...
    if res.success:
        ...
        if abs(c_main2) > 1e-7:
            ...
            fig.add_trace(go.Scatter(x=x_range, y=y_target, mode='lines', 
                                     name=f"Maqsad to'g'ri chizig'i (Z={opt_res:.2f})", 
        ...
        st.success(f"### Natija: X = {opt_x:.2f}, Y = {opt_y:.2f}, Z = {opt_res:.2f}")
        
        pdf_file = create_pdf(opt_x, opt_y, opt_res, obj_type)
        st.download_button("📥 Hisobotni yuklab olish (PDF)", data=pdf_file, file_name="lp_hisobot.pdf", mime="application/pdf")
    else:
