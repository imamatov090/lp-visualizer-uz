...
# --- SIDEBAR: TIL TANLASH (ENG TEPADA) ---
with st.sidebar:
    lang = st.radio("Tilni tanlang / Выберите язык:", ("O'zbekcha", "Русский"), horizontal=True)
    st.markdown("---")
...
if lang == "O'zbekcha":
    ...
    t_opt = "Optimum"
    t_line = "Maqsad to'g'ri chizig'i"
else:
...
def create_pdf(opt_x, opt_y, opt_val, obj_type, current_lang):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", 'B', size=16)
    report_title = "LP Masalasi Hisoboti" if current_lang == "O'zbekcha" else "Otchet resheniya zadachi LP"
    pdf.cell(200, 10, txt=report_title, ln=True, align='C')
    pdf.ln(10)
    pdf.set_font("Arial", size=12)
    pdf.cell(200, 10, txt=f"{t_type}: {obj_type}", ln=True)
...
if solve_btn:
    ...
    if res.success:
        ...
        pdf_file = create_pdf(opt_x, opt_y, opt_res, obj_type, lang)
