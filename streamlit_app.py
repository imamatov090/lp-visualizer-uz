...
if solve_btn:
    ...
    if corner_points:
        ...
        # ICHKI NUQTA (Vnutrennyaya tochka)
        fig.add_trace(go.Scatter(x=[center[0]], y=[center[1]], mode='markers', marker=dict(color='blue', size=10), name="Внутр. точка"))
        
        # ICHKI NUQTA ORQALI O'TUVCHI DARAJA CHIZIG'I (Liniya urovnya)
        z_inner = c_main1 * center[0] + c_main2 * center[1]
        if abs(c_main2) > 1e-7:
            y_inner = (z_inner - c_main1 * x_range) / c_main2
            fig.add_trace(go.Scatter(x=x_range, y=y_inner, mode='lines', line=dict(color='blue', dash='dot'), name=f"Линия уровня (Z={z_inner:.2f})"))
    ...
    if res.success:
        ...
        # OPTIMAL DARAJA CHIZIG'I
        if abs(c_main2) > 1e-7:
            y_target = (opt_res - c_main1 * x_range) / c_main2
            fig.add_trace(go.Scatter(x=x_range, y=y_target, mode='lines', line=dict(color='black', dash='dash'), name=f"Целевая прямая"))

        fig.add_trace(go.Scatter(x=[opt_x], y=[opt_y], mode='markers+text', text=[f"Optimum"], marker=dict(color='gold', size=15, symbol='star'), name="Оптимум"))
...
if st.session_state.history:
    ...
    st.download_button(t_pdf, data=pdf_file, file_name="report.pdf", mime="application/pdf")
