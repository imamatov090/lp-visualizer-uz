...
with st.sidebar:
    ...
    for i, cons in enumerate(st.session_state.constraints):
        ...
        with cl1: a_v = st.number_input(f"a{i}", value=float(cons['a']), key=f"inp_a{i}", label_visibility="collapsed")
        with cl2: b_v = st.number_input(f"b{i}", value=float(cons['b']), key=f"inp_b{i}", label_visibility="collapsed")
        with cl3: op_v = st.selectbox(f"op{i}", ("≤", "≥", "="), index=("≤", "≥", "=").index(cons['op']), key=f"inp_op{i}", label_visibility="collapsed")
        with cl4: c_v = st.number_input(f"c{i}", value=float(cons['c']), key=f"inp_c{i}", label_visibility="collapsed")
        with cl5: 
...
if solve_btn:
    ...
    if res.success:
        ...
        if abs(c_m2) > 1e-7:
            ...
            fig.add_trace(go.Scatter(x=x_range, y=y_target, mode='lines', name="Z line", line=dict(color='black', dash='dash')))

        # Vektor VZ 
        fig.add_annotation(x=opt_x + 1.5, y=opt_y + 1.5, ax=opt_x, ay=opt_y, xref="x", yref="y", axref="x", ayref="y",
                           text="VZ", showarrow=True, arrowhead=3, arrowcolor="red", font=dict(color="red"))
        ...
        st.success(f"### {L['res']}: X = {opt_x:.2f}, Y = {opt_y:.2f}, Z = {opt_res:.2f}") [cite: 1, 9, 15]
        
        pdf_file = create_pdf(opt_x, opt_y, opt_res, obj_t, st.session_state.lang) [cite: 1, 15]
        st.download_button(L['download'], data=pdf_file, file_name="lp_report.pdf", mime="application/pdf") [cite: 1, 15]
    else:
        st.error(L['no_res'])
