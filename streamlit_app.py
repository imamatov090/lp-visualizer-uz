...
# --- GRAFIK VA YECHIM (SKRINSHOTDAGIDEK QAVS ICHIDAGI RAQAMLAR BILAN) ---
if solve_btn:
    ...
    # Cheklov chiziqlari va QAVS ICHIDAGI RAQAMLAR (1), (2)...
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']
    for i, c in enumerate(st.session_state.constraints):
        if abs(c['b']) > 1e-7:
            y_vals = (c['c'] - c['a'] * x_range) / c['b']
            fig.add_trace(go.Scatter(x=x_range, y=y_vals, mode='lines', line=dict(color=colors[i % len(colors)], width=2), name=f"({i+1}) {c['a']}x + {c['b']}y = {c['c']}"))
            
            # Chiziq ustiga aynan (1), (2) raqamini qo'yish
            # Chiziqning grafik chetidagi koordinatasini topamiz
            label_x = -limit + 2 + (i * 1.5) 
            label_y = (c['c'] - c['a'] * label_x) / c['b']
            
            if -limit < label_y < limit:
                fig.add_annotation(
                    x=label_x, y=label_y, 
                    text=f"({i+1})", # Aynan skrinshotdagidek qavs ichida
                    showarrow=False, 
    ...
    if res.success:
        ...
        if norm > 0:
            ...
            fig.add_annotation(x=opt_x + vx, y=opt_y + vy, ax=opt_x, ay=opt_y, xref="x", yref="y", axref="x", ayref="y",
                               text="VZ", showarrow=True, arrowhead=3, arrowsize=1.2, arrowwidth=2.5, arrowcolor="red", font=dict(color="red", size=14))

        # Optimum nuqtasi
        fig.add_trace(go.Scatter(x=[opt_x], y=[opt_y], mode='markers+text', text=[f"Opt({opt_x:.2f}; {opt_y:.2f})"], 
                                 textposition="top right", marker=dict(color='gold', size=14, symbol='star', line=dict(color='black', width=1)), name="Оптимум"))

    # --- O'QLAR VA KOORDINATALAR (SKRINSHOTDAGIDEK) ---
    ...
    # Seriflar (Chiziqchalar) va raqamlar
