...
if st.session_state.lang == "O'zbekcha":
    ...
    t_analysis = "🔍 Masala tahlili (Sezuvchanlik)"
else:
    ...
    t_analysis = "🔍 Анализ задачи (Чувствительность)"
...
if solve_btn:
    ...
    # Hi-Ghs metodidan foydalanish duallikni ko'rish imkonini beradi
    ...
    if res.success:
        ...
        # ODR hisoblash (Kodingizdagi mantiq)
        corner_points = []
        ...
        # Gradient va Optimal nuqta chizish
        if abs(c_main2) > 1e-7:
        ...
        fig.add_trace(go.Scatter(x=[opt_x], y=[opt_y], mode='markers+text', text=[f"Opt ({opt_x:.2f}; {opt_y:.2f})"], marker=dict(color='gold', size=18, symbol='star')))
        ...
        fig.update_layout(plot_bgcolor='white', height=700, xaxis=dict(range=[-15, 15]), yaxis=dict(range=[-15, 15]))
        st.plotly_chart(fig, use_container_width=True)

        # --- SEZUVCHANLIK TAHLILI (YANGI QISM) ---
        st.markdown(f"### {t_analysis}")
        
        # Shadow prices va slack (yetishmovchilik) ma'lumotlarini tayyorlash
        analysis_data = []
        # 'highs' metodi 'ineq' va 'eq' uchun alohida natija beradi
        shadow_prices = res.get('ineqlin', {}).get('marginals', [])
        slacks = res.get('slack', [])

        for i, c in enumerate(st.session_state.constraints):
            status = "Активно" if abs(slacks[i] if i < len(slacks) else 0) < 1e-5 else "Запас"
            analysis_data.append({
                "Ограничение": f"L{i+1}",
                "Тип": c['op'],
                "Ресурс (b)": c['c'],
                "Остаток (Slack)": round(float(slacks[i]), 4) if i < len(slacks) else 0,
                "Статус": status
            })
        ...
        st.session_state.history.insert(0, {
            ...
            'c1': c_main1, 'c2': c_main2, 'x': opt_x, 'y': opt_y, 'z': opt_res, 'type': obj_type,
