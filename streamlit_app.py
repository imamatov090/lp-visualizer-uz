...
# --- PDF FUNKSIYASI (Xatosiz versiya) ---
def create_pdf(opt_x, opt_y, opt_val, obj_type):
    ...
    pdf.cell(200, 10, txt=f"Optimal X: {opt_x:.2f}", ln=True)
    pdf.cell(200, 10, txt=f"Optimal Y: {opt_y:.2f}", ln=True)
    pdf.cell(200, 10, txt=f"Znachenie Z: {opt_val:.2f}", ln=True)
    # Xatolik chiqmasligi uchun faqat lotin harflari ishlatildi
...
if solve and st.session_state.constraints:
    ...
    for i, c in enumerate(st.session_state.constraints):
        ...
        if abs(c['b']) > 1e-9:
            ...
            fig.add_trace(go.Scatter(x=x_range, y=y_vals, mode='lines', name=label))
    ...
    if res.success:
        ...
        # --- SETKA (GRID) SOZLAMALARI ---
        fig.update_layout(
            title="График решения",
            xaxis=dict(showgrid=True, gridwidth=1, gridcolor='LightGrey', zerolinecolor='black', range=[-15, 10]),
            yaxis=dict(showgrid=True, gridwidth=1, gridcolor='LightGrey', zerolinecolor='black', range=[-20, 20]),
            plot_bgcolor='white', # Orqa fon oq, setka ko'rinishi uchun
            height=700
        )
        st.plotly_chart(fig, use_container_width=True)
        
        # PDF yuklab olish tugmasi (Xatosiz versiya)
        pdf_data = create_pdf(opt_x, opt_y, opt_val, obj_type)
